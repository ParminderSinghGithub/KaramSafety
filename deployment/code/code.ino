...
// float agg_input[40];       // mean, std, min, max × 10 sensors
// float features[58];        // agg_input (40) + extracted (18)
// float selected[47];        // selected features
// float final_scaled[47];    // scaled features used for prediction
...
//     // === Compute 40 aggregated features (no scaling!)
//     for (int i = 0; i < 10; ++i) {
//       float sum = 0, min_v = input_batch[i][0], max_v = input_batch[i][0];
//       for (int j = 0; j < SAMPLE_SIZE; ++j) {
//         float val = input_batch[i][j];
...
//     // === Step 2: Append extracted features to get 58D
//     for (int i = 0; i < 40; ++i) features[i] = agg_input[i];
//     extract_all_features(input_batch, features + 40);

//     // === Step 3: Select + Scale + Predict
//     select_features(features, selected);
//     scale_final(selected, final_scaled);
//     int pred = predict_class(final_scaled);
...
// ====== SELECT MODULE ======
#define MODULE_ID 1   // Change to 2 for Module 2
// ===========================
...
#if MODULE_ID == 1
const int udpPort = 12344;
calData calib = {
  true,
  {0.3633, -0.20, -0.2667},
  {2.72, 1.7067, 1.5267},
  {-40.89, 148.87, -1278.49},
  {1.10, 1.01, 0.9067}
};
#elif MODULE_ID == 2
...
#else
#error "Invalid MODULE_ID. Please set to 1 or 2."
#endif
...
void print_features_udp(WiFiUDP &udp, const char* udp_ip, int udp_port, float features[58]) {
    char line[120];

    for (int i = 0; i < 58; ++i) {
        snprintf(line, sizeof(line), "F[%02d]=%.6f\n", i, features[i]);
        udp.beginPacket(udp_ip, udp_port);
        udp.write((const uint8_t*)line, strlen(line));
        udp.endPacket();
        delay(1);  // Prevent congestion
    }
}
...
void loop() {
  ...
  if (sample_idx >= SAMPLE_SIZE) {
    ...
    // === Feature extraction (40 + 18 = 58)
    for (int i = 0; i < 40; ++i) features[i] = agg_input[i];
    extract_all_features(input_batch, features + 40);
    print_features_udp(udp, udpAddress, udpPort, features);
    ...
    Serial.print("Predicted class (Module ");
    Serial.print(MODULE_ID);
    Serial.print("): ");
    Serial.println(pred);

    char msg[50];
    snprintf(msg, sizeof(msg), "M%d Prediction: %d", MODULE_ID, pred);
